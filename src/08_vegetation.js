...
function buildCrops(scene) {
  ...
  // 番茄：竹竿 + 藤叶 + 红果
  const tomatoFruit = solid(THREE.BufferGeometryUtils.mergeGeometries([0, 1, 2, 3].map(k => { const g = new THREE.IcosahedronGeometry(0.045, 0); g.translate(Math.cos(k * 1.7) * 0.18, 0.45 + k * 0.22, Math.sin(k * 1.7) * 0.18); return g.toNonIndexed(); })), 0xd02a1e);

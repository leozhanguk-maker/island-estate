...
function setupFP(ctx) {
  ...
  const START = { x: 175.3, z: -55.4, yaw: 0 };   // 别墅正北门口（面朝泳池、淡水湖与瀑布）
  ...
  const TP = [
    ['别墅北门', START.x, START.z, 0], ['登岸浮台', 60, 87, Math.PI / 2], ['水闸步道', 30, 126.4, Math.PI / 2, 4.46], ['沙滩', 0, 20, Math.PI],
    ['西山机场', -228, -98, 0.9], ['东峰瞭望塔', L.eastTower.x - 0.98, L.eastTower.z + 2.46, 0.4, L.summitPad.h + 20.13], ['农业区', -40, -8, 0], ['住宅楼', 0, -128, 0],
  ];
  ...
  const prompt = document.getElementById('fpprompt'); let act = null;
  ...
  if (st.seat && st.on) { if (e.code === 'KeyE' || ['KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space'].includes(e.code)) { standUp(); e.stopImmediatePropagation && null; } return; } if (DRIVE.active) { if (e.code === 'KeyC' && DRIVE.active === BOAT) { startCruise(); return; } if (e.code === 'KeyE') exitCar({ teleport }); if (e.code === 'KeyV') DRIVE.cam = DRIVE.cam === 'chase' ? 'seat' : 'chase'; return; } if (st.on && e.code === 'KeyE' && act) { if (act.fn) act.fn(); else { const g = act.go; teleport(g.x, g.z, g.yaw, g.y); } } if (st.on && e.code === 'KeyC' && act && act.cruise && st.mode !== 'swim') act.cruise(); if (st.on && e.code === 'KeyM') st.mapR = st.mapR === 150 ? 400 : 150; });
  st.mapR = 150;

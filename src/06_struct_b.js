...
function buildGateTower() {
  ...
  // 一层闸控室：金属墙板围合，北侧门，室内配电柜与水闸电源开关
  const rh = 2.8, toW = (lx, lz) => [cx + lx, cz + lz];
  ...
  box(I, 2 * S, 0.15, 2 * S, M.metalDark, 0, rh + 0.08, 0);
  ...
  GATE.lamp = box(I, 0.04, 0.12, 0.12, M.red, -S + 0.62, 1.75, -0.5); GATE.lamp.userData.live = true;
  GATE.lever = box(I, 0.06, 0.35, 0.06, M.yellow, -S + 0.65, 1.1, -1.3); GATE.lever.userData.live = true;
  collR(cx - S + 0.35, cz - 1.0, 0.3, 0.75, 0, base + 1);
  INTERACT.push({ x: cx - S + 1.1, z: cz - 1.0, r: 1.2, y: base, label: () => GATE.target > 0.5 ? '关闭水闸（电源开关）' : '打开水闸（电源开关）', fn: () => { GATE.target = GATE.target > 0.5 ? 0 : 1; } });
  COLL.walks.push({ kind: 'rect', x: cx, z: cz, hw: S, hd: S, rot: 0, y: base });
  // 塔内楼梯（从闸控室上到观察室）
  TOWER_ROT = 0; towerStairs(I, toW, base, H - 0.4, 3.4, '闸口');
  const top = cabin(I, 3.6, 2.8, H - 0.4, 0.5, [0, 1.3, -1.1, 1.1]);
  towerTop(cx, cz, 0, base + H - 0.4 + 0.125, 3.6, 3.4);

...
function buildVilla() {
  ...
  wallRun('x', 7, [[-11, -6.8, 'w'], [-6.8, -1.6, 'g'], [-1.6, 0.2, 'w'], [0.2, 2.9, 'o'], [2.9, 11, 'w']]); // 南：入户门洞（正门）
  // 正门：双开外开玻璃门（门洞宽 2.7 m、高 2.9 m，门楣以上为实墙），两扇门向雨篷下外开约 100°
  box(V, 2.7, 0.7, 0.25, M.wallWarm, 1.55, 3.25, 7);
  for (const [hx, sd] of [[0.2, 1], [2.9, -1]]) {
    const g = new THREE.Group(); g.position.set(hx, 0.13, 7.14); g.rotation.y = -sd * 1.75; V.add(g);
    const cx = sd * 0.675;
    box(g, 1.33, 2.76, 0.03, M.glassClear, cx, 1.43, 0);                                            // 玻璃门扇
    for (const fx of [0.04, 1.31]) box(g, 0.08, 2.84, 0.07, M.metalDark, sd * fx, 1.42, 0);          // 竖框
    for (const fy of [0.04, 2.8]) box(g, 1.35, 0.08, 0.07, M.metalDark, cx, fy, 0);                   // 上下框
    box(g, 0.04, 1.2, 0.04, M.alu, sd * 1.18, 1.1, 0.07); box(g, 0.04, 1.2, 0.04, M.alu, sd * 1.18, 1.1, -0.07);   // 内外竖拉手
  ...
  box(V, 4.4, 0.88, 0.62, std(0xf0ede6, 0.5), 5.2, 0.57, 5.6); box(V, 4.5, 0.04, 0.65, M.white, 5.2, 1.03, 5.55);   // 操作台从 x=3.0 起（不挡正门）
  for (let i = 0; i < 6; i++) { box(V, 0.01, 0.8, 0.01, M.metalMid, 3.0 + i * 0.73, 0.55, 5.28); box(V, 0.2, 0.015, 0.02, M.alu, 3.37 + i * 0.73, 0.9, 5.27); }
  box(V, 4.4, 0.8, 0.38, std(0xf0ede6, 0.5), 5.2, 2.6, 5.75); box(V, 0.9, 2.0, 0.7, M.alu, 7.9, 1.13, 5.5);
  box(V, 5.5, 3.45, 0.1, M.wallWarm, 5.75, 0.13 + 3.45 / 2, 6.0);   // 厨房背墙：吊柜挂墙，封住操作台与南墙之间的空当（从正门门洞东侧 x=3.0 起，不挡门）
  collS(180 + 3.0, -46 + 6.0, 180 + 8.5, -46 + 6.0, y0 - 0.3, y0 + 3.4);
  // 电视墙（西墙）
